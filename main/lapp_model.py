...
from core.util import UtSystem, log
...
class LAppModel(L2DBaseModel):
    ...
    def Touch(self, x: float, y: float, s_call=None, f_call=None):
        x, y = self.matrixManager.screenToScene(x, y)
        area_id = self.HitTest(x, y)
        if area_id is not None:
            log.Info(f"Hit area: {area_id}")
            self.StartRandomMotion(area_id, MotionPriority.NORMAL, s_call, f_call)
    ...
    def StartRandomMotion(self, name=None, priority=MotionPriority.IDLE, s_call=None, f_call=None):
        if name is None:
        ...
        no = int(random() * count)
        self.StartMotion(name, no, priority, s_call, f_call)

    def StartMotion(self, name, no, priority, s_call=None, f_call=None):
