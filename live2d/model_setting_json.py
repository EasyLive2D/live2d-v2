...
class ModelSettingJson:
    ...
    def getMotionFile(self, name, n):
        if self.json.get(self.MOTION_GROUPS, None) is None or self.json[self.MOTION_GROUPS].get(name, None) is None or \
                self.json[self.MOTION_GROUPS][name][n] is None:
            return None
        return self.json[self.MOTION_GROUPS][name][n][self.FILE]

    def getMotionSound(self, name, n):
        if self.json.get(self.MOTION_GROUPS, None) is None or self.json[self.MOTION_GROUPS].get(name, None) is None or \
                self.json[self.MOTION_GROUPS][name][n] is None or self.json[self.MOTION_GROUPS][name][n].get(self.SOUND,
                                                                                                             None) is None:
            return None
    ...
    def getMotionFadeIn(self, name, n):
        if self.json.get(self.MOTION_GROUPS, None) is None or self.json[self.MOTION_GROUPS].get(name, None) is None or \
                self.json[self.MOTION_GROUPS][name][n] is None or self.json[self.MOTION_GROUPS][name][n].get(
                self.FADE_IN, None) is None:
            return 1000
    ...
    def getInitPartsVisibleID(self, n):
        if self.json.get(self.INIT_PARTS_VISIBLE, None) is None or self.json[self.INIT_PARTS_VISIBLE].get(n,
                                                                                                          None) is None:
            return None
