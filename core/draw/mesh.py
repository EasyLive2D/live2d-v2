...
from core.param import PivotManager
from core.type import Int16Array, Float32Array
...
class Mesh(IDrawData):
    ...
    def read(self, br):
        ...
        if br.getFormatVersion() >= DEF.LIVE2D_FORMAT_VERSION_V2_8_TEX_OPTION:
    ...
    def init(self, aL):
        ...
        if aM == DEF.VERTEX_TYPE_OFFSET0_STEP2:
            if DEF.REVERSE_TEXTURE_T:
                for aJ in range(self.pointCount - 1, 0 - 1, -1):
                    aO = aJ << 1
                    self.uvs[aO + 1] = 1 - self.uvs[aO + 1]
        elif aM == DEF.VERTEX_TYPE_OFFSET2_STEP5:
            for aJ in range(self.pointCount - 1, 0 - 1, -1):
                aO = aJ << 1
            ...
            if self.needTransform():
                ...
                if dc.tmpDeformerIndex < 0:
                    ...
                    if d is not None and not dctx.isOutsideParam():
                        d.transformPoints(mc, dctx, dc.interpolatedPoints, dc.transformedPoints, self.pointCount, DEF.VERTEX_OFFSET, DEF.VERTEX_STEP)
                        dc.available = True
                    else:
                        dc.available = False
    ...
    def draw(self, aN, aK, aI):
        if not (self == aI.getDrawData()):
            raise RuntimeError("context not match")

        if aI.paramOutside[0]:
            return
        ...
        aH = self.getOpacity(aI) * aI.partsOpacity * aI.baseOpacity
        aM = aI.transformedPoints if (aI.transformedPoints is not None) else aI.interpolatedPoints
        aN.setClipBufPre_clipContextForDraw(aI.clipBufPre_clipContext)
        aN.setCulling(self.culling)
        aN.drawTexture(nr, 3 * self.polygonCount, self.indexArray, aM, self.uvs, aH, self.colorCompositionType, aI)
